...
import { GetAssignedIssuesHandler } from './get-assigned-issues.handler';
...
describe('GetAssignedIssuesHandler', () => {
  ...
  describe('successful retrieval', () => {
    test('should retrieve and format assigned issues', async () => {
      ...
      expect(result.data).toContain('| Key | Summary | Status | Priority | Updated |');
    });
  ...
  describe('formatting and display', () => {
    ...
    test('should escape markdown special characters in issue data', async () => {
      ...
      expect(result.data).toContain('Issue with | pipe and * asterisk characters');
    });
  });
}); 