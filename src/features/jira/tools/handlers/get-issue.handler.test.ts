...
import { GetIssueHandler } from './get-issue.handler';
...
describe('GetIssueHandler', () => {
  ...
  describe('successful issue retrieval', () => {
    ...
    test('should handle issue with complex ADF description', async () => {
        ...
        fields: {
          ...
          description: {
            ...
            content: [
              {
                type: 'paragraph',
                content: [{ type: 'text', text: 'This issue demonstrates complex ADF formatting.' }]
              },
              {
                type: 'codeBlock',
          ...
          assignee: { displayName: 'Jane Assignee', emailAddress: 'jane@company.com' },
      ...
      jiraApiMocks.mockJiraApiSuccess('/rest/api/3/issue/TEST-456', complexIssue);
    ...
    test('should handle issue with different priorities', async () => {
      ...
      jiraApiMocks.mockJiraApiSuccess('/rest/api/3/issue/TEST-789', highPriorityIssue);

      const result = await handler.handle({ issueKey: 'TEST-789' }) as McpResponse<string>;
    ...
    test('should handle issue with different statuses', async () => {
      ...
      const result = await handler.handle({ issueKey: 'TEST-101' }) as McpResponse<string>;
    ...
    test('should include assignee and reporter information', async () => {
      ...
      const result = await handler.handle({ issueKey: 'TEST-202' }) as McpResponse<string>;
    ...
    test('should include creation and update timestamps', async () => {
      ...
      const result = await handler.handle({ issueKey: 'TEST-303' }) as McpResponse<string>;
    ...
    test('should handle labels if present', async () => {
      ...
      const result = await handler.handle({ issueKey: 'TEST-404' }) as McpResponse<string>;
  ...
  describe('formatting and display', () => {
    test('should format markdown properly', async () => {
      ...
      const result = await handler.handle({ issueKey: 'FORMAT-1' }) as McpResponse<string>;
    ...
    test('should handle missing optional fields gracefully', async () => {
      ...
      const result = await handler.handle({ issueKey: 'MIN-1' }) as McpResponse<string>;
    ...
    test('should handle string description (legacy format)', async () => {
      ...
      jiraApiMocks.mockJiraApiSuccess('/rest/api/3/issue/STR-1', issueWithStringDesc);

      const result = await handler.handle({ issueKey: 'STR-1' }) as McpResponse<string>;
