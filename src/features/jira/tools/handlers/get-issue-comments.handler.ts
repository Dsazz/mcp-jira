...
import type { Comment, GetCommentsOptions } from "../../api/jira.models.types";
import { CommentsFormatter, type CommentsContext } from "../../formatters/comments.formatter";
import { getIssueCommentsSchema, type GetIssueCommentsParams } from "../utils/schemas";
...
> {
  ...
  protected async execute(params: GetIssueCommentsParams): Promise<string> {
    try {
      ...
      this.logger.info(`Getting comments for JIRA issue: ${validatedParams.issueKey}`);
      ...
      const allComments = await this.client.getIssueComments(validatedParams.issueKey, apiOptions);

      // Apply client-side filtering for advanced parameters
      const filteredComments = this.applyAdvancedFiltering(allComments, validatedParams);
