...
import { SearchIssuesHandler } from './search-issues.handler';
...
describe('SearchIssuesHandler', () => {
  ...
  beforeEach(() => {
    ...
    mockClient = {
      searchIssues: async (jql: string, fields?: string[], maxResults?: number) => {
  ...
  describe('JQL query building', () => {
    test('should use direct JQL when provided', async () => {
      ...
      const result = await handler.handle({ jql: 'project = "TEST" AND status = "To Do"' }) as McpResponse<string>;
  ...
  describe('search results handling', () => {
    test('should handle empty search results', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.data).toContain('No issues found matching your search criteria');
    });
  ...
  describe('formatting and display', () => {
    ...
    test('should include issue summaries and statuses', async () => {
      const mockIssue = testDataBuilder.issueWithStatus('In Progress', 'yellow');
