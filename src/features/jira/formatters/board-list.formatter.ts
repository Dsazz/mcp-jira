...
import type { Board } from "../api/jira.client.types";

/**
 * Format a list of boards for display
 */
export function formatBoardList(
  boards: Board[],
  appliedFilters?: {
    type?: string;
  ...
  if (boards.length === 0) {
    return formatEmptyBoardList(appliedFilters);
  }

  const sections: string[] = [];

  // Header with summary
  sections.push(formatBoardListHeader(boards, appliedFilters));

  // Board list
  sections.push(formatBoardEntries(boards));

  // Pagination info if more results available
  if (paginationInfo?.hasMore) {
    sections.push(formatPaginationInfo(paginationInfo));
  }

  // Footer with next actions
  sections.push(formatBoardListFooter());
 ...
 */
function formatBoardListHeader(
  boards: Board[],
  appliedFilters?: {
    type?: string;
    projectKeyOrId?: string;
  ...
  if (appliedFilters) {
    ...
    if (appliedFilters.name) filterParts.push(`name: "${appliedFilters.name}"`);
 ...
 */
function formatBoardEntries(boards: Board[]): string {
  const entries = boards.map((board, index) => {
    const sections: string[] = [];

    // Board header with type badge
    const typeBadge = getTypeBadge(board.type);
 ...
 */
function formatBoardListFooter(): string {
  const sections: string[] = [];
 ...
 */
function formatEmptyBoardList(appliedFilters?: {
  type?: string;
  projectKeyOrId?: string;
  name?: string;
  state?: string;
 ...
 */
function formatPaginationInfo(paginationInfo: {
  hasMore?: boolean;
  maxResults?: number;
  startAt?: number;
}): string {
